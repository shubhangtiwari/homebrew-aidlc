<!-- generated from .ai/ -- do not edit by hand. Run `make init cursor` to regenerate. -->
...
- `architect` — `.cursor/agents/architect.md` — Medium/large/uncertain planning after main-session classify-change triage (spec + approval brief). Use when next is draft-spec or the user asks for design or a spec.
- `implementer` — `.cursor/agents/implementer.md` — Edits source within an approved plan and flags layer or contract violations rather than working around them. Use when the user is ready to apply changes that already have a plan or spec.
- `reviewer` — `.cursor/agents/reviewer.md` — Mandatory step 3 after implementer for medium/large governed changes (approved spec). Also use when the user explicitly asks for a review. Not part of trivial/small workflow.

## Cursor rules

- `core.mdc` — always applied (routing, spec gate, delegation)
- `governance-spec-gate.mdc` — globs: `{**/*,tests/**,docs/spec/**,docs/blueprints/**,docs/adr/**,docs/ARCHITECTURE.md,docs/architecture/**}`
- `persona-architect.mdc` — same globs as governance rule
- `persona-implementer.mdc` — same globs as governance rule
- `persona-reviewer.mdc` — same globs as governance rule

## Cursor skills

Agent Skills under `.cursor/skills/<name>/SKILL.md` (from `.ai/skills/` via `make init cursor`).
Invoke with `/` + skill name or let Agent decide from the skill description.

- `classify-change` — `.cursor/skills/classify-change/SKILL.md` — Mandatory triage before governed implementation. Main session runs this skill (read playbook, no subagent). Auto-delegate architect only when tier is medium, large, or uncertain.
- `init-architecture` — `.cursor/skills/init-architecture/SKILL.md` — Initializes or refreshes repository architecture documentation by analyzing the codebase and recommending a reference profile. Use when the user asks to initialize the project, initialize architecture, bootstrap governance, set up layer rules, or when docs/ARCHITECTURE.md is missing, empty, or still the generic template without project-specific facts.
- `orchestrate-spec` — `.cursor/skills/orchestrate-spec/SKILL.md` — Executes an approved spec by work-package waves, delegating one implementer per work package per wave. Use when a spec has work_packages and parallel execution is desired after status is approved.
